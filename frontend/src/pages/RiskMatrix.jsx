...
import { Grid3x3, AlertTriangle, Sparkles, Loader2 } from "lucide-react";
...
export default function RiskMatrix({ companyId }) {
  ...
  const load = () => api.get(`/companies/${companyId}/risks`).then(r => setRisks(r.data));
  useEffect(() => { load(); }, [companyId]);

  const genAI = async () => {
    if (!confirm("Gerar riscos com IA a partir do Diagnóstico? Eles entram na Matriz e no Plano de Ação.")) return;
    setBusy(true);
    try {
      const { data } = await api.post(`/companies/${companyId}/risks/ai-generate`);
      for (let i = 0; i < 90; i++) {
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-4">
          ...
          {isStaff && (
            <button onClick={genAI} disabled={busy} className="inline-flex items-center gap-2 px-4 py-2 bg-belota-gold text-black text-sm font-semibold rounded hover:opacity-90 disabled:opacity-50">
              {busy ? <Loader2 className="w-4 h-4 animate-spin" /> : <Sparkles className="w-4 h-4" />} Gerar riscos com IA
            </button>
          )}
        </div>
      </div>
